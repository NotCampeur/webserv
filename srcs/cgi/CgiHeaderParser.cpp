...
CgiHeaderParser::parse_char(char c)
{
	...
	{
		Logger(Arguments::get_instance().log_file(), error_type, error_lvl) << "Max cgi header file size reached";
	...
	switch (_state)
	{
		...
		case FIELD_NAME :
		{
			if (std::iscntrl(c))
			{
				Logger(Arguments::get_instance().log_file(), error_type, error_lvl) << "Cgi header parsing: control character found in cgi header field name \"" << _field_name << "\"";
				throw HttpException(StatusCodes::BAD_REQUEST_400);
			}
			else if(c == ':')
			{
				...
				{
					Logger(Arguments::get_instance().log_file(), error_type, error_lvl) << "Cgi header parsing: whitespace found in field name: \"" << _field_name << "\"";
					throw HttpException(StatusCodes::BAD_REQUEST_400);
				}
				_state = COLON;
			}
