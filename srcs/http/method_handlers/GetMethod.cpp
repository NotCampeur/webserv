...
GetMethod::handle(Request & req, Response & resp)
{
	...
	if (resp.path_is_dir())
	{
		...
		else if (req.get_server_config().get_autoindex() == false)
		{
			if (req.get_server_config().get_default_file_dir() != NULL)
			{
				resp.set_path(*req.get_server_config().get_default_file_dir());
			}
			else
			{
