...
RequestBodyParser::parse_char(char c)
{
	switch (_state)
	{
		case START :
		{
			...
			{
				std::string encoding = _request.headers().find("transfer-encoding")->second;
				for (size_t i = 0; i < encoding.size(); ++i)
				{
					encoding[i] = std::tolower(encoding[i]);
				}
				if (encoding.find("chunked") != std::string::npos) // TBU -> need to make case-insensitive check
					_state = CHUNK_SIZE;
				else
			...
			else if (_request.headers().find("content-length") != _request.headers().end())
			{
				...
				if (_size > _request.server_config().max_client_body_size() || _size == ULONG_MAX)
				{
